            ...
            if (itemStack.getType() == ItemType.NAVIGATION_COUNTDOWN.getType()) {
                ...
                if (leftTime <= 0) {
                    ...
                    Util.sendTitleForPlayerInfoList(
                            ...
                            beQuestioned.getPlayer().getDisplayName()
                    );
                    game.broadcast(
                            WhoIsTheSpyLocale.PREFIX,
                            WhoIsTheSpyLocale.QUESTIONER_SELECT_BE_QUESTIONED_TIMEOUT,
                            "%QUESTIONER%",
                    ...
                    if (WhoIsTheSpy.getInstance().getConfig().getConfiguration().isInt("ItemSlot.END_ANSWER")) {
                        beQuestioned.getPlayer().getInventory().setItem(
                                WhoIsTheSpy.getInstance().getConfig().getConfiguration().getInt("ItemSlot.END_ANSWER"),
