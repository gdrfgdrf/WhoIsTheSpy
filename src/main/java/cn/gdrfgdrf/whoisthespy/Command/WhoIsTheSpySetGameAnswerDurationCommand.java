...
public class WhoIsTheSpySetGameAnswerDurationCommand extends SubCommand {
    public static final String SYNTAX;

    static {
        SYNTAX = "/who setAnswerDuration %START%%NAME%%END% %START%%INTEGER%%END%"
    ...
    @Getter
    private final LocaleString description = WhoIsTheSpyLocale.COMMAND_SET_GAME_ANSWER_DURATION;

    public WhoIsTheSpySetGameAnswerDurationCommand(WhoIsTheSpy whoIsTheSpy) {
        super(false, 3, 3, "setAnswerDuration", WhoIsTheSpyCommand.PERMISSION_ADMINISTRATOR_PREFIX + "setAnswerDuration", whoIsTheSpy);
    }
        ...
        if (Util.verifyStringToInteger(args[2])) {
            if (Integer.parseInt(args[2]) > 0) {
                if (!game.isEnabled()) {
                    ...
                    WhoIsTheSpyLocale.SUCCESS_SET_GAME_ANSWER_DURATION.message(WhoIsTheSpyLocale.PREFIX, sender, "%GAME%", args[1], "%TIME%", args[2]);
                } else {
