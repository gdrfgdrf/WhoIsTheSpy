...
public class WhoIsTheSpySetGameDurationCommand extends SubCommand {

    public static final String SYNTAX;

    static {
        SYNTAX = "/who setDuration %START%%NAME%%END% %START%%INTEGER%%END%"
    ...
    @Getter
    private final LocaleString description = WhoIsTheSpyLocale.COMMAND_SET_GAME_DURATION;

    public WhoIsTheSpySetGameDurationCommand(WhoIsTheSpy whoIsTheSpy) {
        super(false, 3, 3, "setDuration", WhoIsTheSpyCommand.PERMISSION_ADMINISTRATOR_PREFIX + "setDuration", whoIsTheSpy);
    }
    ...
    public void onCommand(CommandSender sender, String[] args) {
        ...
        if (Util.verifyStringToInteger(args[2])) {
            if (Integer.parseInt(args[2]) > 0) {
                if (!game.isEnabled()) {
                    game.setGameDuration(Integer.parseInt(args[2]));
                    WhoIsTheSpyLocale.SUCCESS_SET_GAME_DURATION.message(WhoIsTheSpyLocale.PREFIX, sender, "%GAME%", args[1], "%TIME%", args[2]);
                } else {
