...
public class WhoIsTheSpySetGameSelectBeQuestionedDurationCommand extends SubCommand {
    public static final String SYNTAX;

    static {
        SYNTAX = "/who setSelectBeQuestionedDuration %START%%NAME%%END% %START%%INTEGER%%END%"
    ...
    @Getter
    private final LocaleString description = WhoIsTheSpyLocale.COMMAND_SET_GAME_SELECT_BE_QUESTIONED_DURATION;

    public WhoIsTheSpySetGameSelectBeQuestionedDurationCommand(WhoIsTheSpy whoIsTheSpy) {
        super(false, 3, 3, "setSelectBeQuestionedDuration", WhoIsTheSpyCommand.PERMISSION_ADMINISTRATOR_PREFIX + "setSelectBeQuestionedDuration", whoIsTheSpy);
    }
    ...
    public void onCommand(CommandSender sender, String[] args) {
        ...
        if (Util.verifyStringToInteger(args[2])) {
            if (Integer.parseInt(args[2]) > 0) {
                if (!game.isEnabled()) {
                    game.setSelectBeQuestionedDuration(Integer.parseInt(args[2]));
                    WhoIsTheSpyLocale.SUCCESS_SET_GAME_SELECT_BE_QUESTIONED_DURATION.message(WhoIsTheSpyLocale.PREFIX, sender, "%GAME%", args[1], "%TIME%", args[2]);
                } else {
