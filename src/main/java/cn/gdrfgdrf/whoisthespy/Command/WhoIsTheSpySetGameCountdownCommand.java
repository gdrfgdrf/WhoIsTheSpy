...
public class WhoIsTheSpySetGameCountdownCommand extends SubCommand {
    ...
    @Getter
    private final LocaleString description = WhoIsTheSpyLocale.COMMAND_SET_GAME_COUNTDOWN;

    public WhoIsTheSpySetGameCountdownCommand(WhoIsTheSpy whoIsTheSpy) {
        super(false, 3, 3, "setCountdown", WhoIsTheSpyCommand.PERMISSION_ADMINISTRATOR_PREFIX + "setCountdown", whoIsTheSpy);
    }
    ...
    public void onCommand(CommandSender sender, String[] args) {
        ...
        if (Util.verifyStringToInteger(args[2])) {
            if (Integer.parseInt(args[2]) > 0) {
                if (!game.isEnabled()) {
                    game.setCountdown(Integer.parseInt(args[2]));
                    WhoIsTheSpyLocale.SUCCESS_SET_GAME_COUNTDOWN.message(WhoIsTheSpyLocale.PREFIX, sender, "%GAME%", args[1], "%TIME%", args[2]);
                } else {
