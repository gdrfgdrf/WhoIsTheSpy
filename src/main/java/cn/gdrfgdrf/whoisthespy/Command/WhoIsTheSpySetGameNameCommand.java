...
public class WhoIsTheSpySetGameNameCommand extends SubCommand {
    ...
    static {
        SYNTAX = "/who setName %START%%OLD_NAME%%END% %START%%NEW_NAME%%END%"
                ...
                .replace("%OLD_NAME%", WhoIsTheSpyLocale.OLD_NAME.toString())
                .replace("%NEW_NAME%", WhoIsTheSpyLocale.NEW_NAME.toString());
    }

    @Getter
    private final LocaleString description = WhoIsTheSpyLocale.COMMAND_SET_GAME_NAME;

    public WhoIsTheSpySetGameNameCommand(WhoIsTheSpy whoIsTheSpy) {
        super(false, 3, 3, "setName", WhoIsTheSpyCommand.PERMISSION_ADMINISTRATOR_PREFIX + "setName", whoIsTheSpy);
    }
    ...
    public void onCommand(CommandSender sender, String[] args) {
        ...
        if (game1 != null) {
            WhoIsTheSpyLocale.ERROR_NAME_ALREADY_EXIST.message(WhoIsTheSpyLocale.PREFIX, sender, "%NAME%", args[2]);
            return;
        }
        ...
        if (Util.isValidFileName(args[2])) {
            if (!game.isEnabled()) {
                ...
                game.getGameDataManager().setBeforeRenameFile(game.getGameDataManager().getFile());
                ...
                WhoIsTheSpyLocale.SUCCESS_SET_GAME_NAME.message(WhoIsTheSpyLocale.PREFIX, sender, "%OLD_NAME%", args[1], "%NEW_NAME%", args[2]);
            } else {
