...
public class SetGameMaxPlayerCommand extends SubCommand {
    ...
    public SetGameMaxPlayerCommand(WhoIsTheSpy whoIsTheSpy) {
        super(
                false,
                3,
                3,
                "setMaxPlayer",
                WhoIsTheSpyCommand.PERMISSION_ADMINISTRATOR_PREFIX + "setMaxPlayer",
                whoIsTheSpy
        );
    }
            ...
            if (Integer.parseInt(args[2]) >= 0) {
                if (Integer.parseInt(args[2]) >= game.getMinPlayer()) {
                    if (!game.isEnabled()) {
                        ...
                        WhoIsTheSpyLocale.SUCCESS_SET_GAME_MAX_PLAYER.message(
                                WhoIsTheSpyLocale.PREFIX,
                                sender,
                                "%GAME%",
                                args[1],
                    ...
                    WhoIsTheSpyLocale.ERROR_MUST_MORE_THAN_OR_EQUALS_MIN_PLAYER.message(
                            WhoIsTheSpyLocale.PREFIX,
                            sender,
                            "%MIN%",
                            String.valueOf(game.getMinPlayer())
                    );
                }
            } else {
