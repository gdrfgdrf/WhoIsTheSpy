...
public class WhoIsTheSpyHelpCommand extends SubCommand {
    ...
    public WhoIsTheSpyHelpCommand(WhoIsTheSpy whoIsTheSpy) {
        super(false, 1, 1, "help", WhoIsTheSpyCommand.PERMISSION_USER_PREFIX + "help", whoIsTheSpy);
    }
    ...
    public void onCommand(CommandSender sender, String[] args) {
        ...
        for (SubCommand command : WhoIsTheSpyCommand.commands) {
            if (!command.getPermission().startsWith(WhoIsTheSpyCommand.PERMISSION_USER_PREFIX))
                continue;
        ...
        if (sender.hasPermission(WhoIsTheSpyCommand.PERMISSION_ADMINISTRATOR_PREFIX)) {
            sender.sendMessage(template.replace("%SYNTAX%", WhoIsTheSpyAdminHelpCommand.SYNTAX).replace("%DESCRIPTION%", WhoIsTheSpyLocale.COMMAND_ADMIN.toString()));
        }
