...
public class WhoIsTheSpySetGameMinPlayerCommand extends SubCommand {
    ...
    public WhoIsTheSpySetGameMinPlayerCommand(WhoIsTheSpy whoIsTheSpy) {
        super(false, 3, 3, "setMinPlayer", WhoIsTheSpyCommand.PERMISSION_ADMINISTRATOR_PREFIX + "setMinPlayer", whoIsTheSpy);
    }
            ...
            if (Integer.parseInt(args[2]) >= 0) {
                if (Integer.parseInt(args[2]) >= 3) {
                    if (!game.isEnabled()) {
                        ...
                        WhoIsTheSpyLocale.SUCCESS_SET_GAME_MIN_PLAYER.message(WhoIsTheSpyLocale.PREFIX, sender, "%GAME%", args[1], "%MIN%", args[2]);

                        if (game.getMinPlayer() > game.getMaxPlayer()) {
                            game.setMaxPlayer(Integer.parseInt(args[2]) + 1);
                            WhoIsTheSpyLocale.SUCCESS_SET_GAME_MIN_PLAYER_WITH_MAX_PLAYER.message(WhoIsTheSpyLocale.PREFIX, sender, "%MIN%", args[2]);
                        }
                    } else {
                        WhoIsTheSpyLocale.ERROR_GAME_IS_ENABLED.message(WhoIsTheSpyLocale.PREFIX, sender);
                    }
                } else {
                    WhoIsTheSpyLocale.ERROR_MUST_MORE_THAN_OR_EQUALS_INTEGER.message(WhoIsTheSpyLocale.PREFIX, sender, "%INTEGER%", "3");
                }
            } else {
