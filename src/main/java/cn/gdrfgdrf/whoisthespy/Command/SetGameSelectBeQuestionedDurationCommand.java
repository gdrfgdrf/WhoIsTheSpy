...
public class SetGameSelectBeQuestionedDurationCommand extends SubCommand {
    ...
    public SetGameSelectBeQuestionedDurationCommand(WhoIsTheSpy whoIsTheSpy) {
        super(
                false,
                3,
                3,
                "setSelectBeQuestionedDuration",
                WhoIsTheSpyCommand.PERMISSION_ADMINISTRATOR_PREFIX + "setSelectBeQuestionedDuration",
                whoIsTheSpy
        );
    }
        ...
        if (Util.verifyStringToInteger(args[2])) {
            if (Integer.parseInt(args[2]) > 0) {
                if (!game.isEnabled()) {
                    ...
                    WhoIsTheSpyLocale.SUCCESS_SET_GAME_SELECT_BE_QUESTIONED_DURATION.message(
                            WhoIsTheSpyLocale.PREFIX,
                            sender,
                            "%GAME%",
                            args[1],
                ...
                if (Integer.parseInt(args[2]) == 0) {
                    WhoIsTheSpyLocale.ERROR_IS_ZERO_INTEGER.message(
                            WhoIsTheSpyLocale.PREFIX,
                            sender
                    );
                    return;
